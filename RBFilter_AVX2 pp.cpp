...
bool CRBFilterAVX2::initialize(int width, int height, int thread_count, bool pipelined)
{
	...
	int round_up = ALIGN_SIZE * thread_count;
	if (width % round_up)
	{
	...
	if (pipelined)
	{
		m_stage_buffer[1] = (unsigned char*)_aligned_malloc(m_reserved_width * m_reserved_height * 4, ALIGN_SIZE);
		if (!m_stage_buffer[1])
			return false;
	}
	...
	if (m_pipelined)
	{
		m_horizontal_thread = std::thread(&CRBFilterAVX2::threadFunctionHorizontal, this);
		m_vertical_thread = std::thread(&CRBFilterAVX2::threadFunctionVertical, this);
		//m_horizontal_threads = new std::thread[m_thread_count];
		//m_vertical_threads = new std::thread[m_thread_count];

		//for (int i = 0; i < m_thread_count; i++)
		//{
		//	m_horizontal_threads[i] = std::thread(&CRBFilterAVX2::threadFunctionHorizontal, this, i);
		//	m_vertical_threads[i] = std::thread(&CRBFilterAVX2::threadFunctionVertical, this, i);
		//}
	}
...
void CRBFilterAVX2::release()
{
	if (m_stage_buffer[0])
	{
		_aligned_free(m_stage_buffer[0]);
		m_stage_buffer[0] = nullptr;
	}

	if (m_stage_buffer[1])
	{
		_aligned_free(m_stage_buffer[1]);
		m_stage_buffer[1] = nullptr;
	}
	...
	if (m_pipelined)
	{
//		delete[] m_horizontal_threads;
//		delete[] m_vertical_threads;
	}
...
bool CRBFilterAVX2::filterPipePush(unsigned char* out_data, const unsigned char* in_data, int width, int height, int pitch)
{
	...
	{
		std::lock_guard<std::mutex> lk(m_mutex_stage1);

		m_new_data_width = width;
		m_new_data_height = height;
		m_new_data_pitch = pitch;
		m_new_data_in_buffer = in_data;
		m_new_data_out_buffer = out_data;
		m_new_data_stage_buffer = m_stage_buffer[m_submitted_image_count % 2];
		m_submitted_image_count++;

		m_has_new_data_stage1 = true;
	}
...
void CRBFilterAVX2::threadFunctionHorizontal()
{
	// Wait until filterPipePush() sends data
	{
		std::unique_lock<std::mutex> lk(m_mutex_stage1);
		bool& new_data = m_has_new_data_stage1;
		m_condition1.wait(lk, [&new_data] {return new_data; });

		int				width = m_new_data_width;
		int				height = m_new_data_height;
		int				pitch = m_new_data_pitch;
		const unsigned char* in_buffer = m_new_data_in_buffer;
		unsigned char*	out_buffer = m_new_data_out_buffer;
		unsigned char*	stage_buffer = m_new_data_stage_buffer;

		m_has_new_data_stage1 = false;
	}
	m_condition1.notify_one();
}

void CRBFilterAVX2::threadFunctionVertical()
{
	// wait until horizontal pass sumbits work
	{
		std::unique_lock<std::mutex> lk(m_mutex_stage2);
		bool& new_data = m_has_new_data_stage2;
		m_condition2.wait(lk, [&new_data] {return new_data; });

		int				width = m_stage2_width;
		int				height = m_stage2_height;
		int				pitch = m_stage2_pitch;
		const unsigned char*	in_buffer = m_stage2_in_buffer;
		unsigned char*	out_buffer = m_stage2_out_buffer;
	}
